...
export default {
  ...
  watch: {
    modelValue(newVal) {
      if (this.aceEditor && this.aceEditor.getValue() !== newVal) {
  ...
  mounted() {
    ...
    // ✅强制覆盖worker地址，指向本地静态资源，不再访问七牛CDN
    ace.config.setModuleUrl('ace/mode/javascript_worker', `${ACE_BASE_PATH}/worker-javascript.js`)
    ace.config.setModuleUrl('ace/mode/json_worker', `${ACE_BASE_PATH}/worker-json.js`)
    ace.config.setModuleUrl('ace/mode/css_worker', `${ACE_BASE_PATH}/worker-css.js`)
